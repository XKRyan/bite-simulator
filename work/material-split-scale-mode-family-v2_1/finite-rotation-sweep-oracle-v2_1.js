...
// A target-local endpoint evaluation has 44 scalar add/multiply operations
// (two positions, two angles, two rotations and the intervening vector work).
// We reserve another 52 arithmetic error sources for evaluation order,
// derivative-envelope evaluation and sign changes, giving a fixed 96 budget.
// This budget is deliberately fixed: polygon edge/coordinate counts belong to
// the polygon boolean guard and may not amplify a single transform's error.
// Math.sin/cos/hypot are deliberately *not* claimed to be correctly rounded by
// ECMAScript.  This work-only checkpoint pins the runtime but publishes no
// production numerical authority; a certified trig/norm provider remains a
// hard production gate.
const TRANSFORM_ARITHMETIC_OPERATION_BUDGET = 96;
const TRIG_PROVIDER_CERTIFIED = false;
...
function multiplyUp(left, right) {
  const value = left * right;
  if (value === 0) return left !== 0 && right !== 0 ? Number.MIN_VALUE : 0;
  return nextUp(value);
}
function normUp(value) { return nextUp(Math.hypot(value.x, value.y)); }
function gammaForOperationBudget(operationBudget) {
  if (!Number.isSafeInteger(operationBudget) || operationBudget <= 0) {
    throw new Error('transform arithmetic operation budget must be a positive safe integer');
  }
  const numerator = multiplyUp(operationBudget, Number.EPSILON);
  if (!(Number.isFinite(numerator) && numerator > 0 && numerator < 1)) {
    throw new Error('transform arithmetic gamma numerator is outside the finite domain');
  }
  const gamma = nextUp(numerator / (1 - numerator));
  if (!(Number.isFinite(gamma) && gamma > 0)) {
    throw new Error('transform arithmetic gamma is not finite');
  }
  return gamma;
}
...
// Geometry-neutral first pass used by the v2.1 mode-family composer.  It keeps
// two distinct scales:
//   * finalCoordinateScaleUpperBound bounds only coordinates that can reach a
//     polygon operation (remaining geometry and target-local path endpoints);
//   * transformOperandScaleUpperBound bounds the larger intermediate operands
//     whose cancellation/rounding is covered by a separate gamma_n guard.
// Mixing the second scale into the area term would square a harmless common
// world offset and can manufacture a false geometry-domain stop.
function measureNumericalScales(config) {
  try {
    ...
    if (fileSha256(POLYGON_KERNEL_PATH) !== EXPECTED_POLYGON_KERNEL_SHA256) {
      throw new Error('polygon kernel hash mismatch');
    }
    ...
    if (!timeFractions || timeFractions.length < 2 || timeFractions.length > 4097
      ...
      || timeFractions.some((entry, index) => entry < 0 || entry > 1
        || (index && entry <= timeFractions[index - 1]))) {
    ...
    let finalCoordinateScaleUpperBound = 0;
    let transformOperandScaleUpperBound = 0;
    const includeTransformPoint = (entry) => {
      transformOperandScaleUpperBound = Math.max(
        transformOperandScaleUpperBound, Math.abs(entry.x), Math.abs(entry.y),
      );
    };
    geometryInput.forEach((polygon) => polygon.forEach((ring) => ring.forEach((entry) => {
      finalCoordinateScaleUpperBound = Math.max(
        finalCoordinateScaleUpperBound, Math.abs(entry[0]), Math.abs(entry[1]),
      );
      transformOperandScaleUpperBound = Math.max(
        transformOperandScaleUpperBound, Math.abs(entry[0]), Math.abs(entry[1]),
      );
    })));
    includeTransformPoint(segment.start); includeTransformPoint(segment.end);
    includeTransformPoint(feature.startPosition); includeTransformPoint(material.startPosition);
    ...
    for (const p of pDomain) for (const fraction of timeFractions) {
      const elapsed = h * fraction;
      const featureTranslation = scale(affineVectorAt(feature.linearVelocity, p), elapsed);
      const materialTranslation = scale(affineVectorAt(material.linearVelocity, p), elapsed);
      const featurePosition = add(feature.startPosition, featureTranslation);
      const materialPosition = add(material.startPosition, materialTranslation);
      includeTransformPoint(featureTranslation); includeTransformPoint(materialTranslation);
      includeTransformPoint(featurePosition); includeTransformPoint(materialPosition);
      const featureAngle = feature.startAngle
        + affineScalarAt(feature.angularVelocity, p) * elapsed;
      const materialAngle = material.startAngle
        + affineScalarAt(material.angularVelocity, p) * elapsed;
      for (const localPoint of [segment.start, segment.end]) {
        const rotatedLocal = rotate(localPoint, featureAngle);
        const worldPoint = add(featurePosition, rotatedLocal);
        const materialDifference = subtract(worldPoint, materialPosition);
        const targetLocalPoint = rotate(materialDifference, -materialAngle);
        includeTransformPoint(rotatedLocal); includeTransformPoint(worldPoint);
        includeTransformPoint(materialDifference); includeTransformPoint(targetLocalPoint);
      }
    }
    // Cover interior p/time extrema and the magnitudes of operands that can
    // cancel before the final target-local coordinate is formed. This is a
    // length-scale bound, not merely a sample maximum. In particular it covers
    // large common world offsets, affine intercept/slope cancellation, and an
    // interior rotation whose component exceeds both p-endpoint components.
    const maximumAbsP = Math.max(Math.abs(pDomain[0]), Math.abs(pDomain[1]));
    const motionTranslationBounds = (motion) => {
      const componentBounds = {};
      let operandMaximum = 0;
      for (const axis of ['x', 'y']) {
        const interceptTerm = multiplyUp(h, Math.abs(motion.linearVelocity.intercept[axis]));
        const slopeTerm = multiplyUp(
          multiplyUp(h, Math.abs(motion.linearVelocity.slope[axis])), maximumAbsP,
        );
        componentBounds[axis] = addUp(
          addUp(Math.abs(motion.startPosition[axis]), interceptTerm), slopeTerm,
        );
        operandMaximum = Math.max(
          operandMaximum, interceptTerm, slopeTerm, componentBounds[axis],
        );
      }
      return {
        componentBounds,
        l1Bound: addUp(componentBounds.x, componentBounds.y),
        operandMaximum,
      };
    };
    const featureTranslationBounds = motionTranslationBounds(feature);
    const materialTranslationBounds = motionTranslationBounds(material);
    const localL1Bound = Math.max(
      addUp(Math.abs(segment.start.x), Math.abs(segment.start.y)),
      addUp(Math.abs(segment.end.x), Math.abs(segment.end.y)),
    );
    const worldComponentBounds = {
      x: addUp(featureTranslationBounds.componentBounds.x, localL1Bound),
      y: addUp(featureTranslationBounds.componentBounds.y, localL1Bound),
    };
    const differenceComponentBounds = {
      x: addUp(worldComponentBounds.x, materialTranslationBounds.componentBounds.x),
      y: addUp(worldComponentBounds.y, materialTranslationBounds.componentBounds.y),
    };
    const differenceL1Bound = addUp(
      differenceComponentBounds.x, differenceComponentBounds.y,
    );
    const angularOperandBound = (motion) => addUp(
      addUp(
        Math.abs(motion.startAngle),
        multiplyUp(h, Math.abs(motion.angularVelocity.intercept)),
      ),
      multiplyUp(
        multiplyUp(h, Math.abs(motion.angularVelocity.slope)), maximumAbsP,
      ),
    );
    const featureAngularLengthScale = multiplyUp(localL1Bound, angularOperandBound(feature));
    const materialAngularLengthScale = multiplyUp(
      differenceL1Bound, angularOperandBound(material),
    );
    transformOperandScaleUpperBound = Math.max(
      transformOperandScaleUpperBound,
      featureTranslationBounds.operandMaximum,
      materialTranslationBounds.operandMaximum,
      featureTranslationBounds.l1Bound,
      materialTranslationBounds.l1Bound,
      localL1Bound,
      worldComponentBounds.x,
      worldComponentBounds.y,
      differenceComponentBounds.x,
      differenceComponentBounds.y,
      differenceL1Bound,
      featureAngularLengthScale,
      materialAngularLengthScale,
    );
    transformOperandScaleUpperBound = nextUp(transformOperandScaleUpperBound);
    if (!(Number.isFinite(transformOperandScaleUpperBound)
      && transformOperandScaleUpperBound > 0)) {
      throw new Error('transform operand scale upper bound must be finite and positive');
    }

    // At a fixed time node q(p) is continuously differentiable.  The norm of
    // D(p)=featurePosition-materialPosition is convex because D is affine, so
    // max ||D|| is attained at a p endpoint.  The derivative bound below is
    // ||d|| + |theta_m'| max||D|| + |theta_f'-theta_m'| ||local||.
    // Every scalar component derivative is bounded by the same norm.  Any p is
    // within half the full interval width of one endpoint, hence endpoint
    // component max + L*width/2 covers interior rotational extrema.
    const endpointPDerivativeBound = (fraction, local) => {
      const timeUpper = multiplyUp(h, Math.abs(fraction));
      const linearSlopeDifference = {
        x: nextUp(Math.abs(feature.linearVelocity.slope.x - material.linearVelocity.slope.x)),
        y: nextUp(Math.abs(feature.linearVelocity.slope.y - material.linearVelocity.slope.y)),
      };
      const dSlopeUpper = {
        x: multiplyUp(timeUpper, linearSlopeDifference.x),
        y: multiplyUp(timeUpper, linearSlopeDifference.y),
      };
      const thetaMaterialSlopeUpper = multiplyUp(
        timeUpper, Math.abs(material.angularVelocity.slope),
      );
      const thetaRelativeSlopeUpper = multiplyUp(timeUpper, nextUp(Math.abs(
        feature.angularVelocity.slope - material.angularVelocity.slope,
      )));
      const maxDUpper = Math.max(...pDomain.map((p) => nextUp(normUp(subtract(
        positionAt(feature, p, fraction), positionAt(material, p, fraction),
      )))));
      const first = normUp(dSlopeUpper);
      const second = multiplyUp(thetaMaterialSlopeUpper, maxDUpper);
      const third = multiplyUp(thetaRelativeSlopeUpper, normUp(local));
      const result = addUp(addUp(first, second), third);
      if (!(Number.isFinite(result) && result >= 0)) {
        throw new Error('endpoint p-derivative bound is not finite');
      }
      return result;
    };
    const halfPWidth = multiplyUp(nextUp(pDomain[1] - pDomain[0]), 0.5);
    if (!(Number.isFinite(halfPWidth) && halfPWidth > 0)) {
      throw new Error('p-domain half width is not finite and positive');
    }
    const finalEndpointCertificates = [];
    for (const [timeNodeIndex, fraction] of timeFractions.entries()) {
      for (const [segmentEndpoint, local] of [
        ['start', segment.start], ['end', segment.end],
      ]) {
        const loPoint = relativeEndpointAt(pDomain[0], fraction, local);
        const hiPoint = relativeEndpointAt(pDomain[1], fraction, local);
        const endpointComponentMaximum = nextUp(Math.max(
          Math.abs(loPoint.x), Math.abs(loPoint.y),
          Math.abs(hiPoint.x), Math.abs(hiPoint.y),
        ));
        const derivativeBound = endpointPDerivativeBound(fraction, local);
        const interiorInflation = multiplyUp(derivativeBound, halfPWidth);
        const certifiedComponentUpperBound = addUp(
          endpointComponentMaximum, interiorInflation,
        );
        if (!(Number.isFinite(certifiedComponentUpperBound)
          && certifiedComponentUpperBound >= endpointComponentMaximum)) {
          throw new Error('final coordinate component bound is not finite');
        }
        finalCoordinateScaleUpperBound = Math.max(
          finalCoordinateScaleUpperBound, certifiedComponentUpperBound,
        );
        finalEndpointCertificates.push({
          timeNodeIndex,
          fraction,
          segmentEndpoint,
          endpointComponentMaximum,
          derivativeBound,
          halfPWidth,
          interiorInflation,
          certifiedComponentUpperBound,
        });
      }
    }
    finalCoordinateScaleUpperBound = nextUp(finalCoordinateScaleUpperBound);
    if (!(Number.isFinite(finalCoordinateScaleUpperBound)
      && finalCoordinateScaleUpperBound > 0)) {
      throw new Error('final coordinate scale upper bound must be finite and positive');
    }
    const polygonOperationCount = Math.max(64, geometryInputMetrics.coordinateCount
      + 8 * geometryInputMetrics.edgeCount + 64);
    const transformOperationCount = TRANSFORM_ARITHMETIC_OPERATION_BUDGET;
    const measurementBody = {
      pDomain, h, timeFractions, segment, feature, material,
      remainingGeometry: geometryInput,
      finalCoordinateScaleUpperBound,
      finalCoordinateScalePolicy:
        'remaining coordinates plus outward endpoint-component maximum inflated by a certified continuous p-derivative half-width bound',
      transformOperandScaleUpperBound,
      transformOperandScalePolicy:
        'nextUp-outward continuous p/time and transform-intermediate conservative operand bound',
      finalEndpointCertificates,
      polygonOperationCount,
      transformOperationCount,
      trigProviderCertified: TRIG_PROVIDER_CERTIFIED,
    };
    return deepFreeze({
      ok: true,
      status: 'finite-rotation-split-scales-measured',
      moduleSha256: selfSha256(),
      finalCoordinateScaleUpperBound,
      finalCoordinateScalePolicy: measurementBody.finalCoordinateScalePolicy,
      transformOperandScaleUpperBound,
      transformOperandScalePolicy: measurementBody.transformOperandScalePolicy,
      finalEndpointCertificates,
      polygonOperationCount,
      transformOperationCount,
      measurementSignature: digest('finite-rotation-split-scale-measurement-v2.1', measurementBody),
    });
  } catch (error) {
...
function createFiniteRotationSweepKernel(config) {
  try {
    ...
    const scaleMeasurement = measureNumericalScales(config);
    if (!scaleMeasurement.ok) {
      throw new Error(`split-scale measurement stopped: ${scaleMeasurement.reason}`);
    }
    ...
    let endpointCoordinateScale = 0;
    geometryInput.forEach((polygon) => polygon.forEach((ring) => ring.forEach((entry) => {
      endpointCoordinateScale = Math.max(
        endpointCoordinateScale, Math.abs(entry[0]), Math.abs(entry[1]),
      );
    })));
    allDomainPoints.forEach((entry) => {
      endpointCoordinateScale = Math.max(
        endpointCoordinateScale, Math.abs(entry.x), Math.abs(entry.y),
      );
    });
    const measuredFinalCoordinateScaleUpperBound =
      scaleMeasurement.finalCoordinateScaleUpperBound;
    const measuredTransformOperandScaleUpperBound =
      scaleMeasurement.transformOperandScaleUpperBound;
    if (!(Number.isFinite(measuredFinalCoordinateScaleUpperBound)
      && measuredFinalCoordinateScaleUpperBound > 0)
      || endpointCoordinateScale > measuredFinalCoordinateScaleUpperBound
      || !(Number.isFinite(measuredTransformOperandScaleUpperBound)
        && measuredTransformOperandScaleUpperBound > 0)) {
      throw new Error('internal split-scale measurement drift');
    }
    if (Object.hasOwn(config, 'numericalCoordinateScaleUpperBound')) {
      throw new Error('stale single numericalCoordinateScaleUpperBound is forbidden');
    }
    if (!Object.hasOwn(config, 'finalCoordinateScaleUpperBound')
      || !Object.hasOwn(config, 'transformOperandScaleUpperBound')) {
      throw new TypeError('both split numerical scale upper bounds are required');
    }
    const finalCoordinateScaleUpperBound = finite(
      config.finalCoordinateScaleUpperBound,
      'finalCoordinateScaleUpperBound',
    );
    const transformOperandScaleUpperBound = finite(
      config.transformOperandScaleUpperBound,
      'transformOperandScaleUpperBound',
    );
    if (!(finalCoordinateScaleUpperBound > 0)
      || !(transformOperandScaleUpperBound > 0)) {
      throw new Error('split numerical scale upper bounds must be positive');
    }
    if (finalCoordinateScaleUpperBound < measuredFinalCoordinateScaleUpperBound) {
      throw new Error('finalCoordinateScaleUpperBound under-reports the measured final-coordinate bound');
    }
    if (transformOperandScaleUpperBound < measuredTransformOperandScaleUpperBound) {
      throw new Error('transformOperandScaleUpperBound under-reports the measured transform-operand bound');
    }
    ...
    const polygonOperationCount = Math.max(64, geometryInputMetrics.coordinateCount
      + 8 * geometryInputMetrics.edgeCount + 64);
    if (polygonOperationCount !== scaleMeasurement.polygonOperationCount
      || scaleMeasurement.transformOperationCount
        !== TRANSFORM_ARITHMETIC_OPERATION_BUDGET) {
      throw new Error('internal numerical operation-budget measurement drift');
    }
    const transformOperationCount = TRANSFORM_ARITHMETIC_OPERATION_BUDGET;
    const transformGamma = gammaForOperationBudget(transformOperationCount);
    ...
    const transformLengthGuard = multiplyUp(
      transformGamma, transformOperandScaleUpperBound,
    );
    const transformRoundoffDiameter = multiplyUp(2, transformLengthGuard);
    const computedFinalCoordinateScaleUpperBound = addUp(
      finalCoordinateScaleUpperBound, transformRoundoffDiameter,
    );
    const polygonLengthGuard = multiplyUp(multiplyUp(multiplyUp(
      256, Number.EPSILON,
    ), computedFinalCoordinateScaleUpperBound), polygonOperationCount);
    const accumulatedNumericalLengthFloor = addUp(
      transformLengthGuard, polygonLengthGuard,
    );
    if (![polygonLengthGuard, transformLengthGuard,
      transformRoundoffDiameter, computedFinalCoordinateScaleUpperBound,
      accumulatedNumericalLengthFloor].every((entry) => Number.isFinite(entry) && entry >= 0)) {
      throw new Error('split numerical length guard overflowed');
    }
    const lengthGuard = Math.max(requestedLengthGuard, accumulatedNumericalLengthFloor);
    if (!(lengthGuard <= 1e-6 * computedFinalCoordinateScaleUpperBound)) {
      throw new Error('lengthTolerance is outside the relative domain');
    }
    ...
    let canonicalGeometryCoordinateScale = 0;
    for (const collection of [geometryInput, canonicalGeometry, xorGeometry]) {
      collection.forEach((polygon) => polygon.forEach((ring) => ring.forEach((entry) => {
        canonicalGeometryCoordinateScale = Math.max(
          canonicalGeometryCoordinateScale, Math.abs(entry[0]), Math.abs(entry[1]),
        );
      })));
    }
    const canonicalGeometryLengthGuard = geometryInput.length
      ? 256 * Number.EPSILON * canonicalGeometryCoordinateScale * polygonOperationCount : 0;
    const canonicalGeometryAreaGuard = geometryInput.length ? numericalAreaGuard({
      ...
      coordinateCount: geometryInputMetrics.coordinateCount + canonicalMetrics.coordinateCount
        + xorMetrics.coordinateCount,
      lengthGuard: canonicalGeometryLengthGuard,
      scaleValue: canonicalGeometryCoordinateScale,
    }) : 0;
    const canonicalGeometrySignature = digest('finite-rotation-canonical-geometry-v2', {
      geometry: canonicalGeometry,
      metrics: canonicalMetrics,
      canonicalGeometryCoordinateScale,
      canonicalGeometryLengthGuard,
      canonicalGeometryAreaGuard,
      canonicalGeometrySignatureScope:
        'geometry-only identity telemetry; not an operational roundoff allowance',
    });
    const geometryAreaGuard = geometryInput.length ? numericalAreaGuard({
      ...
      lengthGuard, scaleValue: computedFinalCoordinateScaleUpperBound,
    }) : 0;
    const geometryReferenceArea = Math.max(geometryInputMetrics.area, canonicalMetrics.area,
      computedFinalCoordinateScaleUpperBound ** 2 * Number.EPSILON);
    const relativeAreaCapLimit = RELATIVE_NUMERIC_AREA_CAP * geometryReferenceArea;
    const relativeAreaCapMargin = relativeAreaCapLimit - geometryAreaGuard;
    ...
    const commonNumericalPolicySignature = digest('finite-rotation-common-numerical-policy-v2.1', {
      finalCoordinateScaleUpperBound,
      transformOperandScaleUpperBound,
      transformRoundoffDiameter,
      computedFinalCoordinateScaleUpperBound,
      requestedLengthGuard,
      polygonLengthGuard,
      transformLengthGuard,
      accumulatedNumericalLengthFloor,
      effectiveLengthGuard: lengthGuard,
      effectiveGeometryAreaGuard: geometryAreaGuard,
      polygonOperationCount,
      transformOperationCount,
      transformGamma,
      transformArithmeticModel:
        'work-only pinned-runtime gamma_n for fixed basic arithmetic operations; ECMAScript trig/norm has no platform-independent ULP certificate',
      trigProviderCertified: TRIG_PROVIDER_CERTIFIED,
      productionNumericalAuthority: false,
      relativeNumericAreaCap: RELATIVE_NUMERIC_AREA_CAP,
      relativeLengthDomainScale: 'common computedFinalCoordinateScaleUpperBound',
      areaScalePolicy: 'computed final polygon-coordinate bound (exact/discrete final plus twice transform guard); raw transform operands never enter scaleValue squared',
    });
    const geometrySnapshotSignature = digest('finite-rotation-composite-geometry-snapshot-v2.1', {
    ...
    const pathDescriptor = { pDomain, h, timeFractions, segment, feature, material,
      ...
      measuredFinalCoordinateScaleUpperBound,
      finalCoordinateScalePolicy: scaleMeasurement.finalCoordinateScalePolicy,
      measuredTransformOperandScaleUpperBound,
      transformOperandScalePolicy: scaleMeasurement.transformOperandScalePolicy,
      finalCoordinateScaleUpperBound,
      transformOperandScaleUpperBound,
      transformRoundoffDiameter,
      computedFinalCoordinateScaleUpperBound,
      scaleMeasurementSignature: scaleMeasurement.measurementSignature,
      commonNumericalPolicySignature };
    const pathSignature = digest('finite-rotation-discrete-path-v2.1', pathDescriptor);
    ...
    function boundDiscretePathInterval(loValue, hiValue) {
      try {
        ...
        const halfWidth = multiplyUp(nextUp(pHi - pLo), 0.5);
        ...
        for (let index = 0; index < timeFractions.length - 1; index += 1) {
          ...
          const derivativeRadius = multiplyUp(derivativeBound, halfWidth);
          const radius = addUp(derivativeRadius, transformRoundoffDiameter);
          const midpointPerimeter = generalizedHullPerimeter(cellPoints(middleNodes, index));
          const perimeterUpper = addUp(midpointPerimeter,
            multiplyUp(multiplyUp(2, Math.PI), radius));
          const variation = addUp(
            multiplyUp(multiplyUp(2, perimeterUpper), radius),
            multiplyUp(multiplyUp(2, Math.PI), multiplyUp(radius, radius)),
          );
          geometricAreaVariation = addUp(geometricAreaVariation, variation);
          sweepPerimeterUpper = addUp(sweepPerimeterUpper, perimeterUpper);
          cellProofs.push({ index, derivativeBound, derivativeRadius,
            transformRoundoffDiameter, radius, midpointPerimeter, perimeterUpper, variation });
        }
        const result = {
          ok: true,
    ...
    function boundFreshAreaInterval(loValue, hiValue) {
      try {
        ...
        const intervalSampleGuard = numericalAreaGuard({
          ...
          lengthGuard,
          scaleValue: computedFinalCoordinateScaleUpperBound,
        });
    ...
    const descriptor = deepFreeze({
      schema: 'finite-rotation-polygonal-sweep-kernel-v2.1', moduleSha256: selfSha256(),
      ...
      commonNumericalPolicySignature,
      geometrySnapshotSignature,
      geometrySnapshotComposition: 'canonical geometry plus common numerical policy; excludes mode path',
      pathSignature, pDomain, h, timeFractions, segment,
      ...
      trigProviderCertified: TRIG_PROVIDER_CERTIFIED,
      productionNumericalAuthority: false,
      geometryReferenceArea,
      relativeAreaCapLimit,
      relativeAreaCapMargin,
      scaleMeasurementSignature: scaleMeasurement.measurementSignature,
      numericalScalePolicy:
        'polygon area scale uses the common computed-final bound (discrete exact/final plus twice transform guard); transform and polygon length guards add before the requested floor; excessive bounds domain-stop',
      relativeNumericAreaCap: RELATIVE_NUMERIC_AREA_CAP,
      ...
      authority: 'none; work-only numerical checkpoint, integrity only',
    });
