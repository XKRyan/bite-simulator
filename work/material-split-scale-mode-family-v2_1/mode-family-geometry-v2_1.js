...
const FINITE_ROTATION_PATH = path.resolve(__dirname, './finite-rotation-sweep-oracle-v2_1.js');
const EXPECTED_COULOMB_SHA256 = '8B3058E5833E96D7C83971B6D0C234E6641D0CB98044012DFFFA5DA2268E1A3E';
const EXPECTED_FINITE_ROTATION_SHA256 = '6C55302E10BC1EECB0333D8549A3A55441A5047869CAE83E920B0F6A00C58449';
...
function createModeFamilyGeometry(config) {
  try {
    ...
    const forbiddenScaleFields = [
      'numericalCoordinateScaleUpperBound',
      'numericalCoordinateScaleUpperBoundByMode',
      'perModeNumericalCoordinateScaleUpperBounds',
      'finalCoordinateScaleUpperBound',
      'finalCoordinateScaleUpperBoundByMode',
      'perModeFinalCoordinateScaleUpperBounds',
      'transformOperandScaleUpperBound',
      'transformOperandScaleUpperBoundByMode',
      'perModeTransformOperandScaleUpperBounds',
    ];
    if (forbiddenScaleFields.some((field) => Object.hasOwn(config, field)
      || Object.hasOwn(geometry, field))) {
      throw new Error('caller-supplied split numerical scale bounds are forbidden');
    }
    ...
    function kernelConfigForMode(mode, finalCoordinateScaleUpperBound,
      transformOperandScaleUpperBound) {
      ...
      if (finalCoordinateScaleUpperBound !== undefined
        || transformOperandScaleUpperBound !== undefined) {
        if (finalCoordinateScaleUpperBound === undefined
          || transformOperandScaleUpperBound === undefined) {
          throw new Error('split numerical scale bounds must be supplied together');
        }
        kernelConfig.finalCoordinateScaleUpperBound = finalCoordinateScaleUpperBound;
        kernelConfig.transformOperandScaleUpperBound = transformOperandScaleUpperBound;
      }
      return kernelConfig;
    }
    const scaleMeasurements = new Map();
    for (const mode of modeRanges.keys()) {
      const measurement = finiteRotation.measureNumericalScales(
        kernelConfigForMode(mode),
      );
      if (!measurement.ok) {
        throw new Error(`finite-rotation mode ${mode.modeKey} scale measurement stopped: ${measurement.reason}`);
      }
      if (measurement.moduleSha256 !== EXPECTED_FINITE_ROTATION_SHA256
        || !(Number.isFinite(measurement.finalCoordinateScaleUpperBound)
          && measurement.finalCoordinateScaleUpperBound > 0)
        || !(Number.isFinite(measurement.transformOperandScaleUpperBound)
          && measurement.transformOperandScaleUpperBound > 0)
        || typeof measurement.measurementSignature !== 'string') {
        throw new Error(`finite-rotation mode ${mode.modeKey} returned an invalid scale measurement`);
      }
      scaleMeasurements.set(mode, measurement);
    }
    const finalCoordinateScaleUpperBound = Math.max(
      ...[...scaleMeasurements.values()].map((entry) => entry.finalCoordinateScaleUpperBound),
    );
    const transformOperandScaleUpperBound = Math.max(
      ...[...scaleMeasurements.values()].map((entry) => entry.transformOperandScaleUpperBound),
    );
    if (!(Number.isFinite(finalCoordinateScaleUpperBound)
      && finalCoordinateScaleUpperBound > 0)
      || !(Number.isFinite(transformOperandScaleUpperBound)
        && transformOperandScaleUpperBound > 0)) {
      throw new Error('mode-family common split numerical scale bounds are invalid');
    }
    const kernelCache = new Map();

    function kernelForMode(mode) {
      if (kernelCache.has(mode)) return kernelCache.get(mode);
      const measurement = scaleMeasurements.get(mode);
      if (!measurement) throw new Error('selected mode lost its native coordinate-scale measurement');
      const kernel = finiteRotation.createFiniteRotationSweepKernel(
        kernelConfigForMode(mode, finalCoordinateScaleUpperBound,
          transformOperandScaleUpperBound),
      );
      if (!kernel.ok) throw new Error(`finite-rotation mode ${mode.modeKey} stopped: ${kernel.reason}`);
      if (kernel.descriptor.measuredFinalCoordinateScaleUpperBound
          !== measurement.finalCoordinateScaleUpperBound
        || kernel.descriptor.measuredTransformOperandScaleUpperBound
          !== measurement.transformOperandScaleUpperBound
        || kernel.descriptor.scaleMeasurementSignature !== measurement.measurementSignature
        || kernel.descriptor.finalCoordinateScaleUpperBound
          !== finalCoordinateScaleUpperBound
        || kernel.descriptor.transformOperandScaleUpperBound
          !== transformOperandScaleUpperBound) {
        throw new Error(`finite-rotation mode ${mode.modeKey} lost the measured/common split-scale binding`);
      }
    ...
    const selectedKernelDescriptors = envelopeSegments.map((entry) => {
      ...
      return { index: entry.index, start: entry.start, end: entry.end,
        ...
        canonicalGeometrySignature: descriptor.canonicalGeometrySignature,
        commonNumericalPolicySignature: descriptor.commonNumericalPolicySignature,
        ...
        measuredFinalCoordinateScaleUpperBound:
          descriptor.measuredFinalCoordinateScaleUpperBound,
        finalCoordinateScalePolicy: descriptor.finalCoordinateScalePolicy,
        measuredTransformOperandScaleUpperBound:
          descriptor.measuredTransformOperandScaleUpperBound,
        transformOperandScalePolicy: descriptor.transformOperandScalePolicy,
        scaleMeasurementSignature: descriptor.scaleMeasurementSignature,
        finalCoordinateScaleUpperBound: descriptor.finalCoordinateScaleUpperBound,
        transformOperandScaleUpperBound: descriptor.transformOperandScaleUpperBound,
        transformRoundoffDiameter: descriptor.transformRoundoffDiameter,
        computedFinalCoordinateScaleUpperBound:
          descriptor.computedFinalCoordinateScaleUpperBound,
        polygonLengthGuard: descriptor.polygonLengthGuard,
        transformLengthGuard: descriptor.transformLengthGuard,
        accumulatedNumericalLengthFloor: descriptor.accumulatedNumericalLengthFloor,
        polygonOperationCount: descriptor.polygonOperationCount,
        transformOperationCount: descriptor.transformOperationCount,
        transformGamma: descriptor.transformGamma,
        trigProviderCertified: descriptor.trigProviderCertified,
    ...
    const canonicalGeometrySignatures = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.canonicalGeometrySignature))];
    const commonNumericalPolicySignatures = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.commonNumericalPolicySignature))];
    if (canonicalGeometrySignatures.length !== 1) {
      throw new Error('mode kernels do not share one canonical geometry identity');
    }
    if (commonNumericalPolicySignatures.length !== 1) {
      throw new Error('mode kernels do not share one internally derived numerical policy');
    }
    if (geometrySignatures.length !== 1) throw new Error('mode kernels do not share one geometry snapshot');
    const constructedFinalScaleBounds = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.finalCoordinateScaleUpperBound))];
    const constructedTransformScaleBounds = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.transformOperandScaleUpperBound))];
    const constructedComputedFinalScaleBounds = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.computedFinalCoordinateScaleUpperBound))];
    const maximumMeasuredFinalCoordinateScaleUpperBound = Math.max(
      ...selectedKernelDescriptors.map((entry) => entry.measuredFinalCoordinateScaleUpperBound),
    );
    const maximumMeasuredTransformOperandScaleUpperBound = Math.max(
      ...selectedKernelDescriptors.map((entry) => entry.measuredTransformOperandScaleUpperBound),
    );
    const finalCoordinateScalePolicies = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.finalCoordinateScalePolicy))];
    const transformOperandScalePolicies = [...new Set(selectedKernelDescriptors
      .map((entry) => entry.transformOperandScalePolicy))];
    if (constructedFinalScaleBounds.length !== 1
      || constructedFinalScaleBounds[0] !== finalCoordinateScaleUpperBound
      || constructedTransformScaleBounds.length !== 1
      || constructedTransformScaleBounds[0] !== transformOperandScaleUpperBound
      || constructedComputedFinalScaleBounds.length !== 1
      || maximumMeasuredFinalCoordinateScaleUpperBound !== finalCoordinateScaleUpperBound
      || maximumMeasuredTransformOperandScaleUpperBound !== transformOperandScaleUpperBound
      || finalCoordinateScalePolicies.length !== 1
      || transformOperandScalePolicies.length !== 1) {
      throw new Error('mode kernels do not share both internally certified split-scale bounds');
    }
    const finalCoordinateScalePolicy = finalCoordinateScalePolicies[0];
    const transformOperandScalePolicy = transformOperandScalePolicies[0];
    const computedFinalCoordinateScaleUpperBound = constructedComputedFinalScaleBounds[0];
    const scaleMeasurementSignature = digest('material-mode-family-split-scale-envelope-v2.1', {
      pDomain,
      maximumMeasuredFinalCoordinateScaleUpperBound,
      finalCoordinateScaleUpperBound,
      maximumMeasuredTransformOperandScaleUpperBound,
      transformOperandScaleUpperBound,
      computedFinalCoordinateScaleUpperBound,
      modeMeasurements: selectedKernelDescriptors.map((entry) => ({
        index: entry.index,
        start: entry.start,
        end: entry.end,
        modeKey: entry.modeKey,
        modeOrdinal: entry.modeOrdinal,
        measuredFinalCoordinateScaleUpperBound:
          entry.measuredFinalCoordinateScaleUpperBound,
        measuredTransformOperandScaleUpperBound:
          entry.measuredTransformOperandScaleUpperBound,
        scaleMeasurementSignature: entry.scaleMeasurementSignature,
      })),
      finiteRotationSha256: EXPECTED_FINITE_ROTATION_SHA256,
    });
    const geometrySnapshotSignature = geometrySignatures[0];
    const canonicalGeometrySignature = canonicalGeometrySignatures[0];
    const commonNumericalPolicySignature = commonNumericalPolicySignatures[0];
    const modeFamilyBody = {
      ...
      transformOperandScaleUpperBound,
      computedFinalCoordinateScaleUpperBound,
      scaleMeasurementSignature,
      parentCoulombSha256: EXPECTED_COULOMB_SHA256,
      finiteRotationSha256: EXPECTED_FINITE_ROTATION_SHA256,
    };
    const modeFamilySignature = digest('material-kkt-mode-family-geometry-v2.1', modeFamilyBody);
    ...
    const descriptor = deepFreeze({
      schema: 'material-kkt-mode-family-geometry-v2.1', moduleSha256: selfSha256(),
      parentCoulombSha256: EXPECTED_COULOMB_SHA256,
      finiteRotationSha256: EXPECTED_FINITE_ROTATION_SHA256,
      pDomain, canonicalGeometrySignature, commonNumericalPolicySignature,
      geometrySnapshotSignature, modeFamilySignature,
      maximumMeasuredFinalCoordinateScaleUpperBound,
      finalCoordinateScalePolicy,
      finalCoordinateScaleUpperBound,
      ...
      computedFinalCoordinateScaleUpperBound,
      scaleMeasurementSignature,
      splitScaleBoundOwner:
        'two independent internal maxima over every 8B envelope mode; caller bounds forbidden; final scale is continuous discrete-path output and transform scale is intermediate-operation conditioning',
